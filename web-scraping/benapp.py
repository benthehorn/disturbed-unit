...
def scrape_housing_data(url):
    ...
    for row in rows:
        ...
        # pp.pprint(data)
        if decoded_row[1] in csvNames:
            save_to_file1 = os.path.join(out_dir, decoded_row[1] + '.csv')
            with open(save_to_file1, 'a', encoding='utf-8') as output_file1:
                writer = csv.writer(output_file1)
                writer.writerow(decoded_row)
                print('row written')
        else:
            save_to_file1 = os.path.join(out_dir, decoded_row[1] + '.csv')
            save_to_csv(data, save_to_file1)
            csvNames.append(decoded_row[1])
            print('file appended')
        return data
...
def run():
    ...
    for url in urls_endings:
        scrape_housing_data(BASE_URL + url)
